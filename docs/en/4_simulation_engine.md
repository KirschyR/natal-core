...
- **Eligibility**: panmictic (`n_demes == 1`) and built-in growth modes (0–4).
  Declarative hooks — deterministic mutations, `sample`, `stop_if_*`,
  `set_param`, and `convert` — run on the device at the same event points as the
  CPU engine, for both deterministic and stochastic models. Python callbacks are
  not supported and are rejected. `enable_gpu` raises `ValueError` when the
  model is ineligible, or `RuntimeError` when the extension was built without
  the `gpu` cargo feature; the device path never silently falls back to the CPU.
